...
int main() {
    ...
    // std::future<ull> OddSum = std::async(std::launch::deferred, findOdd, start, end); //1. both running in single thread
    std::future<ull> OddSum = std::async(std::launch::async, findOdd, start, end); //2.both running in different thread
